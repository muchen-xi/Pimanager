...
### 开源发布准备
- 开源发布：MIT License，作者 晨曦 (Chenxi)
- 移除所有个人默认配置（主机名、用户名），替换为通用占位符
- README 全面更新：添加徽章、项目结构、功能列表
- 合规审查报告更新至 v2.2.2，所有修复已应用
...
### 文件管理背景
- 修复：恢复背景图显示 — 位置感知裁剪，每个 Canvas 显示背景图对应区域，双栏视觉连续无变形
- 新增 `BackgroundManager.apply_to_canvas_positioned()` — 容器级适配+逐栏裁剪+缓存

## v2.1.2 (2026-06-19)

### 紧急修复
- 修复：设置页完全无法渲染 — `card.py` 和 `option_menu.py` 的 `create_text_layer(cw, ch, ...)` 创建全画布不透明 RGBA 层，`alpha_composite()` 合成时每一层覆盖前面所有内容，导致只能看到最后一张卡片
